...
{
    //color = min(color, 60.0); 范围已经被lut限制，删除
    ...
    color = max(color,0.0);
    color = ColorGradeSplitToning(color,useACES);//色调分割split toneing
    color = ColorGradingChannelMixer(color);
    color = max(color,0.0);
    color = ColorGradingShadowsMidtonesHighlights(color,useACES);//Shadows Midtones Highlights
...
float3 GetColorGradedLUT (float2 uv, bool useACES = false) {
    ...
    return ColorGrade(_ColorGradingLUTInLogC?LogCToLinear(color):color, useACES);//为了将0-1的颜色支持hdr，我们使用解释为对数c空间在做这个，范围扩大到了0-59
}
// 【Tonemapping】
