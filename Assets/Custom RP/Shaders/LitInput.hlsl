...
float4 GetMask(InputConfig c)
{
    if(c.useMask)
    {
        return SAMPLE_TEXTURE2D(_MaskMap,sampler_BaseMap,c.baseUV);//sampling mods texture :metallic occlusion detail smoothness
    }
    return 1.0;
...
float2 TransformDetailUV(float2 detailUV)
{
    ...
    return detailUV = detailST.xy*detailUV+detailST.zw;
}
...
float4 GetBase (InputConfig c) {
    ...
    if(c.useDetail)
    {
        ...
        map.rgb = lerp(sqrt(map.rgb),detail<0.0?0.0:1.0,abs(detail)*mask);//detail(-1 1)  0的时候保留原值，大于或者小于都变亮或者变暗    伽马空间插值平衡
        map.rgb*=map.rgb;
    }
    return map * color;
}
